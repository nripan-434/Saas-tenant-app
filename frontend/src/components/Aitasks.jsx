              ...
              <div className="flex mt-3 gap-3 custom-scrollbar transition-all duration-300 overflow-x-auto pb-3">
                ...
                {aitasks?.map(task => (
                  <div key={task._id} className="flex flex-col gap-2 overflow-x-auto  justify-between min-w-80 items-center p-3 border hover:bg-[#B6FF3B] bg-[#B6FF3B]/90 text-[#0C1A2B] rounded-xl ">
                    ...
                    <button className='bg-[#0C1A2B] rounded-xl p-1 w-full text-[#B6FF3B]' onClick={() => { setSelectedTask(task) }}>add to task</button>
                    {
                      selectedTask==task ? <div onClick={() => { setSelectedTask(null)  }} className='fixed inset-0 z-10 bg-black/30 backdrop-blur-sm flex justify-center items-center'>
                        <motion.form
                          onSubmit={(e) => {
                            e.preventDefault()
                            dispatch(addaitask({ task: { ...task, dueDate }, projectId }))
                          ...
                          <div>
                            <span className='text-[#B6FF3B]'>Deadline:  </span>
                            <input onChange={handleinput} className='bg-[#B6FF3B]/90 rounded-sm px-3 text-black' min={new Date().toISOString().split("T")[0]} name='date' value={dueDate} type='date' />
